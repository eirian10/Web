...
//    {
//         $query = "SELECT * FROM `baihat` JOIN casi ON casi.id_casi = baihat.id_casi LIMIT $Begin_row, 18";
//         $result = $this -> db -> select($query);
//         return $result;
//    }
...
//    {
//         $query = "SELECT * FROM  `baihat` JOIN casi ON casi.id_casi = baihat.id_casi WHERE theloai = '$Category' LIMIT $Begin_row, 18";
//         $result = $this -> db -> select($query);
//         return $result;
//    }
 ...
 public function searchMusic($search) {
   ...
   {
        $query = "SELECT * FROM `baihat` JOIN casi ON casi.id_casi = baihat.id_casi LIMIT $page, 18";
        $result = $this -> db -> select($query);
        return $result;
   }
   ...
   {
        $query = "SELECT * FROM  `baihat` JOIN casi ON casi.id_casi = baihat.id_casi WHERE theloai = '$Category' LIMIT $page, 18";
        $result = $this -> db -> select($query);
        return $result;
   }
}
